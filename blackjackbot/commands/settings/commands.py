...
async def language_callback(update, context):
    ...
    match = re.search(r"^lang_([a-z]{2}(?:-[a-z]{2})?)$", query_data.strip())
    print(match)
    if match:
        ...
        # Handle the case when the query data doesn't match the expected pattern
        logger.warning("Invalid language query data: {}".format(query_data))
        await update.effective_message.edit_text(text="Invalid language selection.")
